    ...
    $.ajax({
        ...
        success: function () {
            ...
            swal({
                ...
                if (result.value) {
                    window.location.href = '/QuanLy/ChiTietPhieu/Index?orderId=' + $('#orderId').val();
                }
            });
        },
        error: function () {
...
function checkQtt(thisRow) {
    if (thisRow.attr('class') === 'addingRow') {
        $.ajax({
            ...
            success: function (data) {
                var div = '<div style="margin-left: 377px;"><i class="fa fa-eye text-black"></i><a href="/QuanLy/OrderTimeLine/DetailProduct?productID=' + thisRow.find(".productId").val() + '">Xem chi tiết</a></div>'
